...
/* PanelCB ():  串口1配置 												   	*/
/********************************************************************************************/
void	Com1Config(stcUartConfig * sComConfig)
{
	if(ReadCom1ConfigFromFile(sComConfig))		//从文件中读取配置文件有效
		SetCviCtrlCom1Config(sComConfig);		//如果读取的文件有效，在面板上显示
	
	sComConfig->open = 0;						//关闭串口
	
	WriteCom1ConfigToFile(sComConfig);			//将配置信息写入文件
}
...
void	QuitCom1Panel(int	panel)
{
	...
	gsCom1Contrl.sConfig.config = 1;										//配置更改
	
	WriteCom1ConfigToFile(&gsCom1Contrl.sConfig);							//将配置信息写入文件

	cvi_OpenCom(&gsCom1Contrl);
	...
	switch (event)
	{
		case EVENT_COMMIT:
			if(gsCom1Contrl.sConfig.open  == 0)
			{
				...
				ReadCom1ConfigFromFile(&gsCom1Contrl.sConfig);					//从文件中取出配置信息
...
int CVICALLBACK Com1TimerCallback (int panel, int control, int event,
								   void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
