...
/********************************************************************************************/
void	Com1Config(stcUartContrl * sCom)
{
	int status;
...
{
	GetCtrlVal (panel, COM1_PANEL_PORT,  	&gCom1UartConfig.port);		//取端口
	GetCtrlVal (panel, COM1_PANEL_BAUT,  	&gCom1UartConfig.baut);		//取波特率
	GetCtrlVal (panel, COM1_PANEL_BITS,  	&gCom1UartConfig.bits);		//取数据长度
	GetCtrlVal (panel, COM1_PANEL_PARITY, 	&gCom1UartConfig.parity);		//取校验
	GetCtrlVal (panel, COM1_PANEL_STOPS, 	&gCom1UartConfig.stops);		//取停止位

	WriteCom1ConfigToFile(&gCom1UartConfig);								//将配置信息写入文件
...
/* PanelCB ():  OpenCom1														*/
/********************************************************************************************/
void	OpenCom1(stcUartContrl * sCom)
{
	Com1Config(sCom);											//配置串口
	cvi_OpenCom(sCom);											//打开串口
}
...
//通过串口1发送数据
/********************************************************************************************/
void	SendCom1(stcUartContrl * sCom,char *buf,int len)
{
	cvi_SendCom(sCom,buf,len);
}

/********************************************************************************************/
/* PanelCB ():  OpenCom1																	*/
/********************************************************************************************/
void	CloseCom1(stcUartContrl * sCom)
{
	cvi_CloseCom (sCom);												//关闭串口
}
